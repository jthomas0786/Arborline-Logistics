  ...
  return <main className={styles.page}>
    ...
    <section className={styles.pilot} style={{paddingTop:72}}>
      <div className={styles.pilotCopy}>
        ...
        <p>Tell us who your ideal customer is. ArborLine will use that profile to prepare a small sample of matching businesses so you can judge the fit before becoming a client.</p>
        <ul>
      ...
      <form className={styles.form} method="post" action="/api/public/free-sample">
        ...
        <div className={styles.formRow}><label>Company<input name="company" autoComplete="organization" maxLength={180} required/></label><label>Your industry<input name="industry" placeholder="e.g. Commercial cleaning" maxLength={120}/></label></div>
        <div className={styles.formRow}><label>Target geography<input name="serviceArea" placeholder="e.g. Chicagoland, Illinois, Midwest" maxLength={180}/></label><label>Website<input name="website" type="url" placeholder="https://" maxLength={300}/></label></div>
        <label>Who is your ideal customer?<textarea name="targetCustomer" rows={4} maxLength={1400} required placeholder="Example: Medical offices and professional buildings with 10,000+ sq. ft. within 30 miles that use recurring janitorial service."/></label>
        <label>Who usually makes the buying decision?<input name="decisionMakerTitles" maxLength={400} placeholder="Example: Facility Manager, Property Manager, Operations Director"/></label>
        <label>Anything else ArborLine should use when matching?<textarea name="notes" rows={3} maxLength={1000} placeholder="Optional: company size, facility type, exclusions, buying signals, or other preferences."/></label>
    ...
    <section className={styles.markets}>
      <div className={styles.sectionIntro}><p className={styles.kicker}>WHAT HAPPENS NEXT</p><h2>A small proof before a bigger commitment.</h2><p>Your request lands in ArborLine's Growth queue. We review the target profile, prepare a small matching sample, and use the result to show what a full ArborLine Connect campaign could look like.</p></div>
      <div className={styles.marketList}><span>Target profile captured</span><span>Match quality reviewed</span><span>3–5 sample businesses</span><span>No automatic sending</span></div>
    </section>
