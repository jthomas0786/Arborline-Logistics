...
export default async function SourcingPage() {
  ...
  return <AppShell active="Sourcing">
    <header><div><p className="eyebrow">AUTOMATIC DISCOVERY</p><h1>Prospect sourcing</h1><p className="muted">Use a client ICP to discover matching companies, deduplicate them, enforce suppression, score fit, and queue companies that still need decision-maker enrichment.</p></div></header>
    ...
    {provider === "APOLLO" ? <div className="notice">Your Apollo connection is configured for <strong>company discovery only</strong>. ArborLine will save and score matched organizations as <strong>Partial</strong> until a contact-enrichment provider is connected. No outreach is sent without a contact email.</div> : null}
    <section className="split">
      ...
      <article className="panel"><div className="panelHead"><div><p className="eyebrow">PIPELINE</p><h3>What happens automatically</h3></div></div><div className="health"><div><span>Company discovery</span><b>Automated</b></div><div><span>Duplicate check</span><b>Required</b></div><div><span>Suppression check</span><b>Required</b></div><div><span>ICP scoring</span><b>Automatic</b></div><div><span>Decision-maker enrichment</span><b>Next provider</b></div><div><span>Work email enrichment</span><b>Next provider</b></div><div><span>Outreach readiness</span><b>Email required</b></div></div></article>
    </section>
