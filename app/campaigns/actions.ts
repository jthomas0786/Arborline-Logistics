...
export async function sendApprovedOutreachLive(form: FormData) {
  ...
  const pool=getPool();
  const sentToday=await pool.query(`SELECT count(*)::int AS count FROM connect_outreach_messages WHERE status IN ('SENT','DELIVERED') AND sent_at >= (date_trunc('day', now() AT TIME ZONE 'America/Chicago') AT TIME ZONE 'America/Chicago')`);
