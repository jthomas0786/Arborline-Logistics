...
export default async function CampaignsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [clientsResult, draftsResult, approvedResult, sentResult] = await Promise.all([
    ...
    pool.query(`SELECT m.id,m.prospect_id,m.client_id,m.subject,m.body_text,m.recipient_email,m.sender_name,m.sender_email,m.created_at,p.company_name,p.contact_name,p.contact_title,p.qualification_score,c.company_name AS client_company FROM connect_outreach_messages m JOIN connect_prospects p ON p.id=m.prospect_id JOIN connect_clients c ON c.id=m.client_id WHERE m.status='DRAFT' ORDER BY m.created_at DESC LIMIT 50`),
  ...
  return (
    <AppShell active="Campaigns">
      ...
      {staging ? <section className="panel" style={{ marginBottom: 12 }}><strong>{staging === "staged" ? `${stagedCount} prospect${stagedCount === 1 ? "" : "s"} staged for the controlled queue.` : "Select a client before staging a batch."}</strong><p className="muted" style={{ marginBottom: 0 }}>Staging only moves reviewed drafts into QUEUED status. It does not contact a prospect.</p></section> : null}
      ...
      <section className="grid stats">
        ...
        <article className="card"><p>Queued</p><h2>{approvedResult.rows.length}</h2><small>Staged, not necessarily sent</small></article>
      ...
      <section className="panel" style={{ marginBottom: 12 }}>
        ...
        <div className="health">
          <div><span>Sender identity</span><b>Josh Thomas</b></div>
          <div><span>Verified send domain</span><b>mail.arborlineconnect.com</b></div>
          <div><span>Reply routing</span><b>josh@arborlineconnect.com</b></div>
          <div><span>One-click unsubscribe secret</span><b>{hasUnsubscribeSecret ? "Configured" : "Missing"}</b></div>
          <div><span>Business postal address</span><b>{hasPostalAddress ? "Configured" : "Missing"}</b></div>
          <div><span>Daily send cap</span><b>{dailyLimit}/day</b></div>
          <div><span>Master live-send switch</span><b>{liveEnabled ? "ON" : "OFF"}</b></div>
          <div><span>Bounce/complaint suppression</span><b>Automatic</b></div>
        </div>
      ...
      <section className="panel" style={{ marginBottom: 12 }}>
        ...
        <p className="muted">Only QUALIFIED + READY prospects with a verified contact email and clear suppression status are selected.</p>
