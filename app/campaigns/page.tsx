...
import { approveAllDrafts, approveOutreachDraft, generateReadyOutreachDrafts, rejectOutreachDraft, sendApprovedOutreachTest, sendApprovedOutreachLive } from "./actions";
...
export default async function CampaignsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const draftGroups = Array.from(new Map(draftsResult.rows.map((draft) => [draft.client_id, { clientId: draft.client_id, clientCompany: draft.client_company, count: draftsResult.rows.filter((row) => row.client_id === draft.client_id).length }])).values());
