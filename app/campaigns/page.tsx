  ...
  return (
    <AppShell active="Campaigns">
      ...
      <section id="review-queue" className="panel" style={{ marginBottom: 12, scrollMarginTop: 96 }}>
        ...
        <p className="muted" style={{ marginBottom: 14 }}>Drafts can be approved one at a time here or through the reviewed batch control above. Follow-ups are prepared after 2 business days with no reply, but still require human approval. Every approval path rechecks qualification, the exact recipient, verified-contact evidence, reply state, and suppression status before a message can enter the queue.</p>
        {draftsResult.rows.length ? draftsResult.rows.map((draft) => { const isSelected = draft.id === reviewDraft; const isFollowUp = draft.experiment_key === CONNECT_FOLLOW_UP_EXPERIMENT_KEY; return <article id={`draft-${draft.id}`} className="exception" key={draft.id} style={{ alignItems: "flex-start", scrollMarginTop: 96 }}><div className="grow"><strong>{draft.company_name} · {draft.contact_name || draft.recipient_email}</strong><p className="muted">{draft.contact_title || "Decision-maker"} · fit {draft.qualification_score ?? 0}/100 · {draft.client_company}{isFollowUp ? " · FOLLOW-UP" : ""}</p><p><strong>To:</strong> {draft.recipient_email}</p><p><strong>From:</strong> Josh Thomas &lt;josh@mail.arborlineconnect.com&gt;</p><p><strong>Subject:</strong> {draft.subject}</p><p className="muted"><strong>Status:</strong> {isFollowUp ? "FOLLOW-UP · " : ""}{draft.status}{isSelected ? " · selected from Live Prospect Pipeline" : ""}</p><div style={{ whiteSpace: "pre-wrap", lineHeight: 1.6, marginTop: 12 }}>{draft.body_text}</div><div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 14 }}><form action={approveOutreachDraft}><input type="hidden" name="messageId" value={draft.id} /><button type="submit">Approve this draft</button></form><form action={rejectOutreachDraft}><input type="hidden" name="messageId" value={draft.id} /><button type="submit" className="secondary">Reject draft</button></form></div><p className="muted" style={{ marginBottom: 0, marginTop: 10 }}>{isFollowUp ? "This second touch was prepared only because 2+ business days passed with no reply. " : ""}Approval only moves this specific message into the controlled 9 AM queue. It does not send the prospect email immediately.</p></div><span className="status">{isFollowUp ? "FOLLOW-UP · DRAFT" : isSelected ? "DRAFT · SELECTED" : draft.status}</span></article>; }) : <div className="empty">No drafts awaiting review.</div>}
      </section>

      <section className="panel" style={{ marginBottom: 12 }}><div className="panelHead"><div><p className="eyebrow">CONTROLLED SEND LAB</p><h3>Test a queued email without contacting the prospect</h3></div><span className="status">Prospect delivery blocked</span></div><p className="muted">The test reruns qualification, exact-recipient and suppression checks, then sends only to the test address. Sender identity is forced to Josh Thomas.</p>{approvedResult.rows.length ? approvedResult.rows.map((row) => { const isFollowUp = row.experiment_key === CONNECT_FOLLOW_UP_EXPERIMENT_KEY; return <article className="exception" key={row.id} style={{ alignItems: "flex-start" }}><div className="grow"><strong>{row.company_name} · {row.contact_name || row.recipient_email}</strong><p>{row.subject}</p><p className="muted">Intended prospect: {row.recipient_email} · {row.client_company}{isFollowUp ? " · FOLLOW-UP" : ""}</p><form action={sendApprovedOutreachTest} className="form" style={{ marginTop: 12 }}><input type="hidden" name="messageId" value={row.id} /><label>Test recipient<input name="testRecipient" type="email" required placeholder="your-email@example.com" /></label><button type="submit">Send controlled test</button></form>{sendReady ? <form action={sendApprovedOutreachLive} style={{ marginTop: 10 }}><input type="hidden" name="messageId" value={row.id}/><button type="submit" className="secondary">Send to approved prospect</button></form> : null}</div><span className="status">{isFollowUp ? "FOLLOW-UP · QUEUED" : "QUEUED"}</span></article>; }) : <div className="empty">Approve a reviewed draft to enable controlled testing.</div>}</section>

      <section className="panel"><div className="panelHead"><div><p className="eyebrow">CAMPAIGN GUARDRAILS</p><h3>Production sending is compliance-gated</h3></div></div><div className="health"><div><span>Suppression rechecked before send</span><b>Active</b></div><div><span>Signed one-click unsubscribe</span><b>{hasUnsubscribeSecret ? "Ready" : "Blocked"}</b></div><div><span>Physical mailing address footer</span><b>{hasPostalAddress ? "Ready" : "Blocked"}</b></div><div><span>Resend delivery webhooks</span><b>Connected</b></div><div><span>Bounce/complaint auto-stop</span><b>Active</b></div><div><span>Daily rate limit</span><b>{dailyLimit}</b></div><div><span>Bulk approval</span><b>Exact-draft confirmation required</b></div><div><span>Follow-up preparation</span><b>Draft-only · human approval required</b></div></div></section>
    </AppShell>
  );
}