...
export default async function CampaignsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <AppShell active="Campaigns">
      ...
      {staging ? <section className="panel" style={{ marginBottom: 12 }}><strong>Bulk staging is locked.</strong><p className="muted" style={{ marginBottom: 0 }}>Approve each reviewed draft individually. No bulk queue action is currently permitted.</p></section> : null}
      ...
      <section className="grid stats">
        ...
        <article className="card"><p>Queued</p><h2>{approvedResult.rows.length}</h2><small>Individually approved, not necessarily sent</small></article>
      ...
      <section id="review-queue" className="panel" style={{ marginBottom: 12, scrollMarginTop: 96 }}>
        ...
        <p className="muted" style={{ marginBottom: 14 }}>Each message must be reviewed and approved individually. The decision-maker safety gate rechecks qualification, the exact recipient, verified-contact evidence, and suppression status before the draft can enter the queue.</p>
