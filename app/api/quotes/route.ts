...
export async function POST(request: Request) {
  try {
    ...
    const required = ["originCity","originState","destinationCity","destinationState","pickupStart","equipmentType","estimatedMiles"];
    ...
    const pricing = priceQuote({ equipmentType: body.equipmentType, estimatedMiles: Number(body.estimatedMiles), pickupAt: body.pickupStart, weightLbs: Number(body.weightLbs || 0) });
    const origin = resolveLocation(body.originCity, body.originState, { lat: body.originLat === undefined ? undefined : Number(body.originLat), lon: body.originLon === undefined ? undefined : Number(body.originLon) });
    const destination = resolveLocation(body.destinationCity, body.destinationState, { lat: body.destinationLat === undefined ? undefined : Number(body.destinationLat), lon: body.destinationLon === undefined ? undefined : Number(body.destinationLon) });
    ...
    const { rows } = await getPool().query(
      ...
      [reference,body.originCity,body.originState,origin?.lat ?? null,origin?.lon ?? null,body.destinationCity,body.destinationState,destination?.lat ?? null,destination?.lon ?? null,body.pickupStart,body.equipmentType,Number(body.estimatedMiles),Number(body.weightLbs || 0)||null,body.commodity || null,Number(body.cargoValue || 0)||null,pricing.expectedCarrierCost,pricing.targetCarrierRate,pricing.maxCarrierRate,pricing.shipperPrice,pricing.targetMarginPct,JSON.stringify(pricing.pricingNotes)]
    );
