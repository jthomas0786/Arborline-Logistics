...
function questionName(value: string) {
...
export function buildConnectOutreachDraft(prospect: Record<string, unknown>) {
  ...
  const roleContext = title
    ? `Given your role as ${title} at ${sentence(company)} I thought this might be relevant.`
    : `${sentence(company)} It looks like it may be a fit.`;
  return {
...
async function eligibleProspects(clientId: string, limit: number) {
...
export async function previewConnectOutreachDrafts(clientId: string, limit = 25) {
  const { rows } = await eligibleProspects(clientId, limit);
  return {
    dryRun: true,
...
export async function prepareConnectOutreachDrafts(clientId: string, limit = 25) {
  const pool = getPool();
  const { rows } = await eligibleProspects(clientId, limit);
  let generated = 0;
  ...
  for (const prospect of rows) {
    ...
    const result = await pool.query(
      `INSERT INTO connect_outreach_messages
       ...
       AND NOT EXISTS (
         SELECT 1 FROM connect_outreach_messages m
         WHERE m.prospect_id=$1 AND m.status IN ('DRAFT','QUEUED','SENT','DELIVERED')
       )
       RETURNING id`,
      [prospect.id, prospect.client_id, CONNECT_FROM_EMAIL, prospect.contact_email, subject, body, prospect.domain]
    );
    generated += result.rowCount ?? 0;
  }

  return { eligible: rows.length, draftsCreated: generated, messagesSent: 0 };
}
