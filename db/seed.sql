...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM organizations WHERE legal_name='Arborline Demo Carrier One') THEN
    ...
    INSERT INTO carriers(organization_id,usdot_number,mc_number,authority_status,insurance_status,fraud_score,performance_score,verified_at,last_verified_at) VALUES(org_id,'9000001','1900001','ACTIVE','VALID',8,96,now(),now()) RETURNING id INTO carrier_id;
  ...
  IF NOT EXISTS (SELECT 1 FROM organizations WHERE legal_name='Arborline Demo Carrier Two') THEN
    ...
    INSERT INTO carriers(organization_id,usdot_number,mc_number,authority_status,insurance_status,fraud_score,performance_score,verified_at,last_verified_at) VALUES(org_id,'9000002','1900002','ACTIVE','VALID',12,91,now(),now()) RETURNING id INTO carrier_id;
  ...
  IF NOT EXISTS (SELECT 1 FROM organizations WHERE legal_name='Arborline Demo Carrier Three') THEN
    ...
    INSERT INTO carriers(organization_id,usdot_number,mc_number,authority_status,insurance_status,fraud_score,performance_score,verified_at,last_verified_at) VALUES(org_id,'9000003','1900003','ACTIVE','VALID',4,89,now(),now()) RETURNING id INTO carrier_id;
